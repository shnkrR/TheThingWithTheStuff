	...
	SubShader 
	{
		...
		{			
			...
			struct v2f
			{
				...
				half3 vWorldTangent		: TEXCOORD3;
				half3 vWorldBinormal	: TEXCOORD4;
				half3 vViewDir			: TEXCOORD5;								
				FOG_COORD(6)
				PROJ_COORD(7)
			...
			v2f vert(appdata v)
			{
				...
				half2 BumpUV = v.texCoord.xy * 10 * _WaveScale / 1.0  + _WaveOffset.xy;
				_ShaderTime = _ShaderTime;
				o.BumpUV.xy = BumpUV + half2(_ShaderTime.x, _ShaderTime.x);
				o.BumpUV.zw = BumpUV * 0.5 - (half2(_ShaderTime.y, _ShaderTime.y));
			...
			half4 frag(v2f IN) : COLOR
			{
				...
				half3 bump1 = UnpackNormal(tex2D(_BumpMap,  IN.BumpUV.xy)).rgb;				
				half3 bump2 = UnpackNormal(tex2D(_BumpMap2, IN.BumpUV.zw)).rgb;
