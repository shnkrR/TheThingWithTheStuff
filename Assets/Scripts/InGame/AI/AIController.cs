...
//        }
    ...
    {        
        ...
        if (Vector3.Distance(Vector3.zero, transform.position) > (World.WorldRadius / 2.0f))
	...
	void SendHit()
	{
        ...
        if (m_enemyTransform.GetComponent<PlayerController>().IsBlock)
            damage /= 4.0f;
        ...
        m_enemyTransform.GetComponent<PlayerController>().RecieveHit((m_TimesMeleeAttacked == 4));
	}
	...
	public void DoRangedDamage()
	{
        ...
        m_enemyTransform.GetComponent<PlayerController>().m_Impact.Play(true);
        Invoke("StopParticle", 0.5f);
	}

    void StopParticle()
    {
        m_enemyTransform.GetComponent<PlayerController>().m_Impact.Stop(true);
    }
